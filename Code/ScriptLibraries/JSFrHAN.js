...
function mostrarCampos(tipo, callback) {
	...
	var url = application.sWebDbName + "agMostrarCampos?Open&edit=" + (application.bEdicion ? "1" : "0") + "&unique=" + application.sUnique + "&form=" + application.sForm + "&tipoHerramienta=" + tipo;
...
function cargarDatos(callback, tipo){
	var url = application.sWebDbName + "agCargarFormHAN?Open"
...
function crearAccion() {
	...
	if(!aplicaSIG){
		...
		bAlert("Debe seleccionar una opción en el campo: " + label );
		return;
	}
	
	if (!application.bNuevo){
			bConfirm("¿Confirma que desea crear una acción?",function(response){			
				if(response){
					var url;
...
function eliminarAccion(idAccion, dbOrigen){
	bConfirm("¿Confirma que desea eliminar la acción?",function(response){
		if(response){
...
function asociarAccion(){
	if (!application.bNuevo){
		...
		bAlert("Debe guardar el documento para poder asociar la acción.");
	}
}
...
function guardarModalAsociacion(ruta){
	...
	if(objAccion){
		...
		bAlert("Debe seleccionar la acción que desea asociar");
		return false;
	}
}
	...
	if(documento.modalConexion){
		...
		if(application.bEdicion){
			...
			if(documento.sPosEstado != "I"){
				botones += "<a class='btn btn-info' href='"+application.sWebDbName+"0/"+application.sUnid+"' target='_blank'>Abrir herramienta de análisis</a>"	
			}
		...
		parent.adaptarModal("Herramienta de análisis",botones);
	}
}
