...
function mostrarCampos(tipo, callback) {
    ...
    var url = application.sWebDbName +
        "agMostrarCamposBI?Open&edit=" + (application.bEdicion ? "1" : "0") +
...
function crearAccion() {
	var parametros;
	// var aplicaSIG = $("[name='fieldSig']").val(); // TODO: Se debe validar con la usuaria porque hay campos que hacen falta y no estan especificados
	// dentro del documento de requerimiento
	var aplicaSIG = "No";
	if(!aplicaSIG){
		var label = $("[name='fieldSig']").parents("tr").find("td").first().text()
		bAlert("Debe seleccionar una opciónn en el campo: " + label );
		return;
	}
	
	if (!application.bNuevo){
			bConfirm("¿Confirma que desea crear una acciÃ³n?",function(response){			
				if(response){
					var url;
...
function eliminarAccion(idAccion, dbOrigen){
	bConfirm("¿Confirma que desea eliminar la acciÃ³n?",function(response){
		if(response){
